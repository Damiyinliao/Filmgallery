...
<script setup>
import { defineProps, onMounted, reactive, getCurrentInstance } from "vue"
...
let cardInfo = reactive({
  photo_url: "http://img.filmgallery.cn/userimgs/IMG_20230227_161408.jpg",
  card_title: "女朋友拍的第一卷电影卷||Kodak 5203",
  nickname: "大米饮料",
  user_avatar: "http://img.filmgallery.cn/avatar/63f588fba1ed9e504406a03c.webp",
  like: 99
})
function getCardInfo() {
  return  $API.reqCardInfo(props.card_id);
}
onMounted( async ()=>{
   let res =  await getCardInfo();
   cardInfo.photo_url = res.data.photos[0].url;
